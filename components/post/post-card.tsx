...
export function PostCard({ post, showRestaurantInfo = false, onEdit, onDelete, onUpdate, onSheetStateChange, embedded = false, isVisible = false }: PostCardProps) {
  ...
  return (
    <div className={embedded ? "" : "bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow"}>
      ...
      {/* Mention Dropdown - Fixed position above comment input, OUTSIDE BottomSheet */}
      {showMentionDropdown && showComments && user && (
        <div 
          className="fixed left-4 right-4 bg-white border border-gray-200 rounded-2xl shadow-2xl max-h-56 overflow-y-auto"
          style={{ bottom: 'calc(70px + env(safe-area-inset-bottom))', zIndex: 99999 }}
        >
          {loadingFriends ? (
